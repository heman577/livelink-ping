...
import { MessageCircle, Hash, UserPlus, ArrowRight } from 'lucide-react';
        ...
        <div className="lg:col-span-2 space-y-8">
            ...
            <div className="bg-gradient-to-r from-indigo-600 to-blue-500 rounded-[2rem] p-6 md:p-8 text-white shadow-xl shadow-indigo-200 relative overflow-hidden group hover:shadow-2xl hover:scale-[1.01] transition-all duration-300">
                ...
                <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
                    ...
                    <button className="bg-white text-indigo-600 font-bold py-3 px-8 rounded-full shadow hover:bg-indigo-50 hover:shadow-lg active:scale-95 transition-all duration-200 whitespace-nowrap">
                        Find a Buddy
                    </button>
                </div>
            </div>
            ...
            <div>
                ...
                {[
                    ...
                    <div key={idx} className="bg-white p-4 md:p-5 rounded-xl border border-slate-100 flex items-center justify-between hover:border-indigo-200 hover:shadow-md hover:-translate-y-0.5 active:scale-[0.99] transition-all duration-200 cursor-pointer group">
                        <div className="flex gap-4 items-center">
                            ...
                            <div>
                                <h4 className="font-bold text-slate-900 line-clamp-1 group-hover:text-indigo-700 transition-colors">{topic.title}</h4>
                                <div className="flex gap-2 items-center text-xs text-slate-500 mt-0.5">
                                    <span className="font-semibold text-slate-700 bg-slate-100 px-1.5 py-0.5 rounded">{topic.tag}</span>
        ...
        <div className="lg:col-span-1">
            ...
            <div className="grid grid-cols-2 gap-4">
                {[
                    ...
                    <div key={idx} className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm hover:shadow-lg hover:-translate-y-1 active:scale-95 transition-all duration-200 cursor-pointer flex flex-col items-center text-center group">
                        <div className={`w-12 h-12 rounded-full ${group.color} flex items-center justify-center mb-3 group-hover:scale-110 transition-transform duration-300`}>
                            <Hash className="w-6 h-6" />
                        </div>
                        <h4 className="font-bold text-slate-900 text-sm mb-1 leading-tight">{group.name}</h4>
                        <p className="text-xs text-slate-500 font-medium">{group.members} members</p>
                    </div>
                ))}
            </div>
        </div>
